...
var rootCmd = &cobra.Command{
	...
	Long: `gsheets is a CLI tool for the Google Sheets API v4.
...
Authentication uses OAuth 2.0 or service accounts. Credentials are resolved in order:
  1. GSHEETS_ACCESS_TOKEN env var (no refresh — short-lived)
  2. GOOGLE_APPLICATION_CREDENTIALS env var (service account JSON file)
  3. GSHEETS_CREDENTIALS env var (service account JSON file)
  4. Config file (set with: gsheets auth login  OR  gsheets auth set-credentials)
...
func printInfo() {
	...
	fmt.Printf("  config:   %s\n", config.Path())
	fmt.Println()
	fmt.Printf("  GSHEETS_ACCESS_TOKEN           = %s\n", maskOrEmpty(os.Getenv("GSHEETS_ACCESS_TOKEN")))
	...
	fmt.Printf("  GSHEETS_CLIENT_ID              = %s\n", maskOrEmpty(os.Getenv("GSHEETS_CLIENT_ID")))
}
...
func resolveCredentials() (string, int64, api.RefreshFunc, error) {
	...
	if credFile := resolveEnv(
		...
		token, expiry, err := exchangeServiceAccountJWT(credFile, sheetsScope)
		if err != nil {
			return "", 0, nil, fmt.Errorf("service account auth failed: %w", err)
		}
		refreshFn := func() (string, int64, error) {
			return exchangeServiceAccountJWT(credFile, sheetsScope)
		}
	...
	if cfg.CredentialsFile != "" {
		token, expiry, err := exchangeServiceAccountJWT(cfg.CredentialsFile, sheetsScope)
		if err != nil {
			return "", 0, nil, fmt.Errorf("service account auth failed: %w", err)
		}
	...
	return "", 0, nil, fmt.Errorf("not authenticated — run: gsheets auth login\nor set GSHEETS_ACCESS_TOKEN env var\nor set GOOGLE_APPLICATION_CREDENTIALS to a service account file")
}
